...
# # Custom CSS to control layout and scrolling
# st.markdown("""
#     <style>
#         /* Make html and body fill the viewport and prevent overall scrolling */
#         html, body {
#             height: 100%;
#             width: 100%;
#             margin: 0;
#             padding: 0;
#             overflow: hidden; /* This is crucial to prevent the main screen from scrolling */
#         }

#         /* Make the main Streamlit app container fill the viewport */
#         .stApp {
...
#         /* Hide header and footer completely to reclaim vertical space */
#         header, footer {
#             visibility: hidden;
#             height: 0;
#             margin: 0;
...
#         /* This is the main content area that we want to contain */
#         /* It should take full height and then manage overflow internally */
#         .block-container {
#             padding-top: 0.5rem; /* Reduced top padding */
#             flex-grow: 1; /* Allows block-container to take remaining vertical space within stApp */
#             height: 100%; /* IMPORTANT: Makes block-container take available height */
#             max-width: 95vw; /* 95% of viewport width */
#             overflow: hidden; /* The block-container itself should NOT scroll */
#             display: flex;
#             flex-direction: column; /* Organize its children (title, tabs, content) vertically */
#         }

#         /* Prevent Streamlit's title and tab bar from shrinking and ensure they take their space */
#         h1 {
#             # margin-top: 0; /* Remove top margin from title */
#             # margin-bottom: 0.5rem; /* Reduce bottom margin from title */
#             margin: 0px;
#             flex-shrink: 0;
#         }
#         [data-testid="stTabs"] {
#             margin-bottom: 0.5rem; /* Reduce bottom margin of tabs */
#             flex-shrink: 0;
#         }

#         /* Target the content area within the selected tab (where left_col and right_col reside) */
#         div[role="tabpanel"] {
#             flex-grow: 1; /* This tab panel should take the remaining height within block-container */
#             height: 100%; /* IMPORTANT: Ensures tabpanel takes available height */
#             display: flex; /* Make it a flex container for the columns */
#             flex-direction: row; /* Arrange columns horizontally */
#             overflow: hidden; /* Prevent this tab panel from scrolling itself */
#             padding: 0 1rem; /* Adjust horizontal padding if necessary */
#         }

#         /* Ensure columns themselves take full height of their parent (tabpanel) and are flex containers */
#         [data-testid="stColumn"] {
#             display: flex;
#             flex-direction: column;
#             flex-grow: 1; /* Allows columns to expand */
#             height: 100%; /* IMPORTANT: Makes columns take full height of parent */
#             overflow: hidden; /* Columns themselves shouldn't scroll */
#         }

#         /* --- THE CRUCIAL FIX FOR SCROLLING RIGHT PANEL --- */
#         /* Target the internal Streamlit div that holds the actual content within the right column.
#            This is usually the `stVerticalBlock` div. */
#         /* For both tab1 and tab2, this targets the second column's content area. */
#         [data-testid="stColumn"]:nth-of-type(2) > div:first-child {
...
#         /* Adjust margins for various Streamlit elements to reduce overall vertical space */
...
#             margin-bottom: 0px !important; /* Remove bottom margin for tighter layout */
#             margin-top: 0px !important; /* Remove top margin too */
#             padding-top: 0.5rem !important; /* Add a small top padding for subheaders/elements */
#             # padding-bottom: 0.5rem !important; /* Add a small bottom padding for subheaders/elements */
#             padding: 0px !important;
#         }

#         /* Specific adjustment for subheaders to control space */
#         .stSubheader {
#             # margin-top: 0.8rem !important; /* Add a bit more space above subheaders */
#             # margin-bottom: 0.4rem !important; /* Reduce space below subheaders */
#             margin: 0px !important;
#         }

#         /* Specific adjustment for the HR line (st.write("---")) */
#         hr {
#             # margin-top: 0.5rem !important; /* Reduce space above HR */
#             # margin-bottom: 0.5rem !important; /* Reduce space below HR */
#             margin: 0px !important;
...
# st.title("🌱 EcoScan - Sustainable Shopping Assistant")
...
#         manual_barcode = st.text_input("Enter barcode", placeholder="Type barcode...")
...
#         }
#         selected_demo = st.selectbox("🎯 Try a known product", list(demo_barcodes.keys()))
#         if selected_demo:
#             barcode = demo_barcodes[selected_demo]
#             st.info(f"Using barcode: {barcode}")

#         if manual_barcode:
#             barcode = manual_barcode

#     # RIGHT SIDE - This is the panel that should be scrollable
#     with right_col:
#         # Placeholder for the main report content
#         report_container = st.empty()

#         if barcode:
#             # Display loader while fetching and processing
#             with st.spinner('Fetching product details and generating report...'):
#                 time.sleep(1) # Simulate network delay or processing time

#                 res = requests.get(f"https://world.openfoodfacts.org/api/v0/product/{barcode}.json")

#                 # Write content to the placeholder after loading
...
#                             eco_data = prod["ecoscore_data"]["agribalyse"]
                            
#                             # --- Carbon Emission & EcoScore Logic ---
#                             carbon_footprint_100g = None
#                             # 1. Try to get direct 'carbon-footprint_100g'
#                             if "carbon-footprint_100g" in prod:
#                                 try:
#                                     carbon_footprint_100g = float(prod["carbon-footprint_100g"])
#                                 except ValueError:
#                                     pass # Handle cases where it might be non-numeric

#                             # 2. Fallback to ecoscore_data.agribalyse.co2_total if direct field is missing
#                             if carbon_footprint_100g is None and "ecoscore_data" in prod and "agribalyse" in prod["ecoscore_data"]:
#                                 agribalyse_data = prod["ecoscore_data"]["agribalyse"]
...
#                             # Round the carbon footprint for display, if available
...
#                             green_score = prod.get("ecoscore_score", 0)
#                             ecoscore_grade = prod.get("ecoscore_grade", "0")
...
#                             with col1:
#                                 st.markdown(
#                                     f"<span style='color: {grade_color}; font-weight: bold;'>{grade_icon} Eco-Grade: {eco_grade}</span>",
#                                     unsafe_allow_html=True
#                                 )
#                             # --- END UPDATED BADGE LOGIC ---
                            
#                             with col2:
#                                 def generate_pdf(product_name, score, barcode):
#                                     pdf = FPDF()
#                                     pdf.add_page()
...
#                                     pdf.cell(200, 10, txt=f"Green Score: {score}/100", ln=4)
#                                     filename = f"EcoScan_Report_{barcode}.pdf"
#                                     pdf.output(filename)
#                                     return filename

#                                 pdf_file = generate_pdf(name, green_score, barcode)
#                                 with open(pdf_file, "rb") as f:
#                                     b64 = base64.b64encode(f.read()).decode()
#                                     href = f'<a href="data:application/octet-stream;base64,{b64}" download="{pdf_file}"><button class="download-btn">📄 Download PDF</button></a>'
...
#                         st.error("❌ API error.")
#         else:
#             # Display info in the same placeholder when no barcode is entered
#             report_container.info("⬅️ Upload image, pick demo, or enter a barcode to begin.")


# with tab2:
#     # Use a different column ratio for Tab 2 to give more space to the right panel
#     left_panel_col, right_panel_col = st.columns([1, 3]) # Left 1 part, Right 3 parts
...
#         comparison_result_placeholder = st.empty() # Placeholder for results

#         # Initial instruction message for the comparison tab
...
#                 time.sleep(1.5) # Simulate loading for comparison

#                 product1_name = "Product 1"
#                 product1_carbon = None
#                 product2_name = "Product 2"
#                 product2_carbon = None
#                 messages = [] # Initialize a list to hold messages for warnings/errors
...
#                             product1_name = data1["product"].get("product_name", f"Product {barcode1}")
#                             product1_carbon = len(barcode1) * 3 + 10 # Mock calculation
#                         else:
...
#                             product2_name = data2["product"].get("product_name", f"Product {barcode2}")
#                             product2_carbon = len(barcode2) * 3 + 15 # Mock calculation
#                         else:
...
#                     # Display the main comparison result message first
...
#                         # Display the chart below the messages
...
#                             barmode='group',
#                             title_text='Carbon Emissions Comparison', # Title text is now enabled here
...
#                     # Display any collected warnings/errors at the end of the content
#                     for msg in messages:
#                         if "⚠️" in msg:
#                             st.warning(msg)
#                         elif "❌" in msg:
...
PAGE_TITLE = "EcoScan - Sustainable Shopping Assistant"
DEMO_BARCODES_MAP = {
    "Jimjam": "8901063029279",
    "Nutella": "3017620429484",
    "Coca-Cola": "5449000000996",
